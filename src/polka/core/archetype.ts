...
abstract class Archetype {
	...
	protected mount(type: string) {
		...
		const modelQueue = [
			this.pickModel(
				this.pool.filter((m) => m.type === type && m.order === "first"),
			),
		];
		...
		while (modelQueue.length > 0 && modelQueue[0] !== undefined) {
			...
			if (!modelConfig) {
				throw new Error(
					`ERROR @ Archetype: failed to retrieve ${type} Model from queue`,
				);
			}
			...
			if (modelConfig.base) {
				modelConfig.use.baseOn(modelConfig.base);
			}

			modelConfig.use.configure(
				...modelConfig.settings.map((values: any[]) =>
					this.randomize(values),
				),
			);
			...
			if (modelConfig.compats.length > 0) {
				...
				nModelConfig.use.configure(
					...modelConfig.settings.map((p: any[]) =>
						this.randomize(p),
					),
				);
	...
	public clear() {
		...
		const allOtherLayers = paper.project.layers.filter(
			(l) => l.id !== this._frame.id,
		);
